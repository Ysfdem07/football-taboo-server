...
async function processIcon(inputPath, outputPath) {
  ...
  for (let i = 0; i < info.width * info.height; i++) {
    ...
    // Alpha falloff for smooth anti-aliased edge blending on black background
    let alpha = 255;
    if (brightness < 18) {
      alpha = 0;
    } else if (brightness < 60) {
      alpha = Math.round(((brightness - 18) / 42) * 255);
    }
...
async function main() {
  ...
  await processIcon(
    path.join(brainDir, 'football_ultra_icon_1786716367214.jpg'),
    'C:/Dev/FootballTaboo/assets/icons/football_3d_icon.png'
  );

  await processIcon(
    path.join(brainDir, 'music_ultra_icon_1786716467872.jpg'),
